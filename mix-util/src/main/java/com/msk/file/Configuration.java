...
public class Configuration {
    ...
    public static Configuration from(String json) {
        ...
        try {
            ...
            throw new OwnException(ResultEnum.ERROR.getCode(), "json解析异常");
        }
    }
    ...
    public static Configuration from(File file) {
        try {
            ...
            throw new OwnException(ResultEnum.ERROR.getCode(), String.format("配置信息错误，您提供的配置文件[%s]不存在. 请检查您的配置文件.", file.getAbsolutePath()));
        } catch (IOException e) {
            throw new OwnException(ResultEnum.ERROR.getCode(), String.format("配置信息错误. 您提供配置文件[%s]读取失败，错误原因: %s. 请检查您的配置文件的权限设置.",
    ...
    public static Configuration from(InputStream is) {
        try {
            ...
            throw new OwnException(ResultEnum.ERROR.getCode(), String.format("请检查您的配置文件. 您提供的配置文件读取失败，错误原因: %s. 请检查您的配置文件的权限设置.", e));
        }
    }
    ...
    public String getNecessaryValue(String key, Integer errorCode) {
        String value = this.getString(key, null);
        if (StringUtils.isBlank(value)) {
            throw new OwnException(errorCode, String.format("您提供配置文件有误，[%s]是必填参数，不允许为空或者留白 .", key));
        }
    ...
    public Boolean getNecessaryBool(String key, Integer errorCode) {
    ...
    public Character getChar(final String path) {
        ...
        try {
            ...
            throw new OwnException(ResultEnum.ERROR.getCode(), String.format("任务读取配置文件出错. 因为配置文件路径[%s] 值非法，期望是字符类型: %s. 请检查您的配置并作出修改.", path,
                    e.getMessage()));
        }
    }
    ...
    public Boolean getBool(final String path) {
        ...
        if (null == result) {
            ...
            throw new OwnException(ResultEnum.ERROR.getCode(), String.format("您提供的配置信息有误，因为从[%s]获取的值[%s]无法转换为bool类型. 请检查源表的配置并且做出相应的修改.",
                    path, result));
        }
    ...
    public Integer getInt(final String path) {
        ...
        try {
            ...
            throw new OwnException(ResultEnum.ERROR.getCode(), String.format("任务读取配置文件出错. 配置文件路径[%s] 值非法, 期望是整数类型: %s. 请检查您的配置并作出修改.", path,
                    e.getMessage()));
        }
    }
    ...
    public Double getDouble(final String path) {
        ...
        try {
            ...
            throw new OwnException(ResultEnum.ERROR.getCode(), String.format("任务读取配置文件出错. 配置文件路径[%s] 值非法, 期望是浮点类型: %s. 请检查您的配置并作出修改.", path,
                    e.getMessage()));
        }
    }
    ...
    public Object remove(final String path) {
        ...
        if (null == result) {
            throw new OwnException(ResultEnum.ERROR.getCode(), String.format("配置文件对应Key[%s]并不存在，该情况是代码编程错误. 请联系DataX团队的同学.", path));
        }
    ...
    private void setObject(final String path, final Object object) {
        ...
        throw new OwnException(ResultEnum.ERROR.getCode(), String.format("值[%s]无法适配您提供[%s]， 该异常代表系统编程错误, 请联系DataX开发团队!",
    ...
    Object buildObject(final List<String> paths, final Object object) {
        if (null == paths) {
            throw new OwnException(ResultEnum.ERROR.getCode(), "Path不能为null，该异常代表系统编程错误, 请联系DataX开发团队 !");
        }
        ...
        for (int i = paths.size() - 1; i >= 0; i--) {
            ...
            throw new OwnException(ResultEnum.ERROR.getCode(), String.format(
    ...
    Object setObjectRecursive(Object current, final List<String> paths,
        ...
        throw new OwnException(ResultEnum.ERROR.getCode(),
